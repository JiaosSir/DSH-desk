 ...
 * `DSH_DESK_SMOKE=1`、`DSH_HOME=<mkdtemp>`、`DSH_DESK_SIDECAR_CACHE=<mkdtemp>`，
 * 超时 5 分钟，断言 exit 0 且输出含 SMOKE_OK；失败打印 profile 日志尾部。
 * 方案 A：不设 `SIDECAR_ROOT`，让 release exe 走生产链路——从 exe 旁
 * sidecar-dist.tar 解压到临时缓存目录再起宿主。
 */
...
const cache = mkdtempSync(join(tmpdir(), 'dsh-smoke-cache-'))
const child = spawn(exe, [], {
  env: {
    ...process.env,
    DSH_DESK_SMOKE: '1',
    DSH_HOME: home,
    DSH_DESK_SIDECAR_CACHE: cache,
  },
