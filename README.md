...
- **windows系统功能集成**：开机自启开关、快捷键展示、重启dsh、一键打开日志目录、检查更新（安装版启动自动检查，发现新版本可一键静默更新并自动重启）；dsh审批事件会镜像为系统通知。
...
- **升级**：
  - **安装版**：应用**启动时自动检查**新版本，发现新版本时侧边栏出现「下载更新」按钮（可关闭本次提示）；也可在设置区「桌面」分区 → **「检查更新」**手动检查。下载完成后点「立即安装」——应用会自动退出、静默覆盖安装并自动重启；`~/.dsh` 中的 profile、会话、凭证原样保留，升级后首次启动自动刷新引擎版本。
  - **便携版**：应用内检查更新会提示前往 GitHub Releases 下载最新压缩包，解压后覆盖到原目录即可（便携版不做应用内自动更新、不做启动自动检查）。
