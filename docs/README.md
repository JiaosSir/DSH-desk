...
- 桥接：npm 外部插件 `@JiaosSir/dsh-desktop-bridge` 只做特性检测 + Tauri IPC 桥（`window.__DSH_DESKTOP__`），浏览器里自动退化。
...
├─ crates/desktop-core/         # 纯逻辑 crate（无 tauri 依赖）：监督状态机、端口、路径、日志
├─ packages/dsh-desktop-bridge/ # 桥接插件（npm 发布；宿主半部 + 浏览器半部）
...
| Rust（MSVC 目标） | stable | 壳与 desktop-core |
...
cargo test -p desktop-core   # 顺带拉取并编译 Rust 依赖
```
...
```powershell
$side = "D:\project\open-source\dsh-desktop\apps\desktop\src-tauri\sidecar-dist"
...
$env:SIDECAR_ROOT = "D:\project\open-source\dsh-desktop\apps\desktop\src-tauri\sidecar-dist"
pnpm desktop:dev

# 方式 B：直接跑构建产物（更快，无 watcher）
$env:SIDECAR_ROOT = "D:\project\open-source\dsh-desktop\apps\desktop\src-tauri\sidecar-dist"
D:\project\open-source\dsh-desktop\target\debug\dsh-desktop.exe
```
...
`SIDECAR_ROOT` 缺省取安装包资源目录（`resources/sidecar`）；生产包无需设置。

## 测试

```powershell
cargo test -p desktop-core     # 监督状态机/就绪解析/路径/日志（17 例）
cargo clippy -p desktop-core -p dsh-desktop --all-targets -- -D warnings
